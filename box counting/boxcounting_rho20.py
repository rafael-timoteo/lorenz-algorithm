...
# -*- coding: utf-8 -*-
"""
Box-counting (ρ=20, regime não-caótico)

Versão afinada para ρ=20 no sistema de Lorenz (σ=10, β=8/3). Nesta faixa, o
sistema converge para pontos de equilíbrio estáveis; portanto, a geometria de
interesse é o transiente (espirais 3D rumo ao equilíbrio). Para estimar a
Dimensão de Caixa dessa curva com viés reduzido, este script:

- Mantém o transiente (burn=0) e **recorta a cauda** muito próxima do ponto fixo
  (parâmetros `--keep-head` e `--r-quantile-min`).
- Seleciona automaticamente uma **janela-plateau** de slopes com **alvo D≈1.0**.
- Usa **travessia de voxels** (segmentos) + **jitters** de grade com **mediana** por
  escala (robusto a alinhamento do grid).
- Descarta **ε** muito grandes (saturação) e muito pequenos (subamostragem) com
  base no **passo espacial mediano** da trajetória.
- Mesma estrutura de I/O do seu arquivo ρ=26 (outdir opcional; CSV/PNG).

Saídas:
- CSV: <outdir>/lorenz_box_rho_20.csv
- PNG (se `--plot-log`): <outdir>/lorenz_box_rho_20.png

Uso rápido:
  python boxcounting_rho20.py --plot-log
Mais robusto (mais lento):
  python boxcounting_rho20.py --jitters 7 --jitter-steps 12 --plot-log
"""
...
# ==========================
# Pré-processamento: recorte do transiente tardio
# ==========================
def clip_transient(points: np.ndarray, keep_head: float, r_quantile_min: float) -> np.ndarray:
    """Mantém a fração inicial e remove pontos muito próximos do equilíbrio.
    - keep_head: fração inicial (0<keep_head≤1)
    - r_quantile_min: remove ||x|| abaixo deste quantil (0–1)
    """
    n = len(points)
    if n == 0:
        return points
...
# ==========================
# Box counting (travessia de voxels)
# ==========================
@dataclass
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Box-Counting 3D para Lorenz — versão ajustada para ρ=20 (não-caótico)")

    # Parâmetros do sistema
    p.add_argument("--rho", type=float, default=20.0)
    p.add_argument("--sigma", type=float, default=10.0)
    p.add_argument("--beta", type=float, default=8.0 / 3.0)

    # Integração: passo pequeno, sem burn, longo o bastante para formar a espiral
    ...
    p.add_argument("--r-quantile-min", type=float, default=0.12, help="remove pontos com ||x|| abaixo deste quantil (0–1)")
