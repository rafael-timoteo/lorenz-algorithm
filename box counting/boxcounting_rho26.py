...
# -*- coding: utf-8 -*-
"""
Box-counting (ρ=26, ajuste para vizinhança do regime caótico)

Esta variante é afinada para ρ=26, onde o sistema de Lorenz já costuma exibir
comportamento caótico (embora janelas quase-periódicas possam ocorrer). O
objetivo é estimar D_box ≈ 2 com robustez, usando:
...
- Jitter de grade com agregação por mediana (reduz artefatos de fase do grid);
- Seleção de janela-plateau por slopes locais mirando D≈2; se falhar, tenta
  data quase-periódica) e, por fim, usa fallback central;
- Recorte automático de escalas (ε) para evitar saturação (ε muito grande)
  e subamostragem (ε menor que ~2× passo espacial mediano da trajetória).

Saídas:
- CSV: rho26/lorenz_box_rho_26.csv
- PNG (se --plot-log): rho26/lorenz_box_rho_26.png

Uso rápido:
  python boxcounting_rho26.py --plot-log
Mais robusto (mais lento):
  python boxcounting_rho26.py --jitters 7 --rot --whiten --plot-log
"""
...
def pick_plateau_near_target(slopes,
                             target=2.0,
                             tol=0.25,
                             edge=3,
                             min_width=3):
    """
    Escolhe uma janela [i0, i1) onde os slopes ficam perto de 'target'
    (± tol), ignorando 'edge' pontos nas extremidades.
    Se não achar, usa uma janela centrada no índice mais próximo de target.
    """
    m = len(slopes)
    ...
    i = edge
    while i < m - edge:
        if abs(slopes[i] - target) <= tol:
            j = i
            while j < m - edge and abs(slopes[j] - target) <= tol:
                j += 1
            length = j - i
            if length >= min_width and length > best_len:
                best_len = length
                best_i0, best_i1 = i, j
            i = j
        else:
    ...
    # fallback: se não achou um platô "largo", usa o ponto mais perto do target
    if best_i0 is None:
        k_star = int(np.argmin(np.abs(slopes - target)))
        best_i0 = max(edge, k_star - 1)
        best_i1 = min(m - edge, k_star + 2)
...
def run_pipeline(
    ...
    thin: int,
    do_whiten: bool,
    force_mid_window: bool,  # mantido só para compatibilidade da CLI
    tol: float,              # idem
) -> FitResult:
    ...
    # integra sempre cheio, faz thin depois
    traj = rk4_integrate(
        x0=x0,
        dt=dt,
        n_steps=total_steps,
    ...
    # === MESMA ESTRATÉGIA DE ESCALAS DO SCRIPT DE rho=28 ===
    ks = np.linspace(k_min, k_max, n_scales, dtype=float)
    eps = L / (2.0 ** ks)   # sem recorte "useful_eps_mask"

    log("== Box counting em múltiplas escalas ==")
    eps, N = box_count_over_scales(
        pts,
        eps,
    ...
    # === DIMENSÃO = MÁXIMO DO LOCAL SLOPE (igual rho=28) ===
        # === DIMENSÃO = MÉDIA EM UM PLATÔ PERTO DE 2.0 ===
    if len(slopes) > 0:
        # escolhe janela "bem comportada" perto de 2.0
        i0, i1 = pick_plateau_near_target(
            slopes,
            target=2.0,
            tol=0.25,    # pode reduzir p/ 0.2 se quiser mais rígido
            edge=3,
            min_width=3
        )
        D_est = float(np.mean(slopes[i0:i1]))
        # índice "representativo" no meio da janela
        k_star = int((i0 + i1) // 2)
    else:
        i0, i1 = 0, 1
        k_star = 0
    ...
    log(
        f"Dimensão estimada (platô near 2.0): "
        f"D ≈ {D_est:.4f} (média de slopes[{i0}:{i1}])"
    )
    log(
        f"Janela destacada no gráfico: "
        f"[{i0}, {i1})  (eps ~ {eps[i1-1]:.4g} a {eps[i0]:.4g})"
    )
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Box-Counting 3D para Lorenz — versão ajustada para ρ=26")

    # Parâmetros do sistema
    p.add_argument("--rho", type=float, default=26.0)
    ...
    p.add_argument("--outdir", type=str, default="rho26", help="diretório de saída para CSV/PNG")
